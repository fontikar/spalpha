...
Here we are removing duplicated values and missing values in coordinate fields.
...
Can't create alpha hulls with fewer than 3 obs, so here we are exclude species that have fewer than 3 records.
...
# Error in unnest(select(myg_nested_ahull, scientificName, points_sf, ahulls_sf),  : 
#   unused argument (cols = c(points_sf, ahulls_sf))
myg_unnested <- myg_nested_ahull |> 
  select(scientificName, points_sf, ahulls_sf) |> 
  tidyr::unnest(cols = c(points_sf, ahulls_sf)) 
# Error in list_unchop(col, ptype = col_ptype) : 
#   VECTOR_ELT() can only be applied to a 'list', not a 'logical'
# for (i in 1:nrow(myg_nested_ahull)) {
#   print(i)
#   myg_unnested_i <- myg_nested_ahull |> 
#     slice(i) |>
#     select(scientificName, points_sf, ahulls_sf) |> 
#     tidyr::unnest(cols = c(points_sf, ahulls_sf))
# }
myg_unnested <- myg_nested_ahull |> 
  select(scientificName, points_sf, ahulls_sf) |> 
  tidyr::unnest(cols = c(points_sf)) |>
  tidyr::unnest(cols = c(ahulls_sf))
# drops species with null for ahulls_sf
...
myg_hull_plot <- with_taxonomic_info |> 
  ...
  select(-occ) |>
  tidyr::unnest(c(points_sf, ahulls_sf)) # getting an error here
